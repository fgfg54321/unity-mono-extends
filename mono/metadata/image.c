...
char dllEncryptFiles[2048];
char* dllEncryptFilesArr[8];
int  encrptCount;
int  enableEncrypt;
...
{   
    ...
    jclass   classID      = (*env)->FindClass(env, "com/game/jni/JNIHelper");
    ...
    if(enableEncrypt)
    {
    	methodID        = (*env)->GetStaticMethodID(env, classID, "getEncryptFiles", "()Ljava/lang/String;");
	jstring encryptFiles    = (jstring)(*env)->CallStaticObjectMethod(env, classID, methodID);
	strtmp          = (char *)(*env)->GetStringUTFChars(env, encryptFiles, NULL);
	int len         = strlen(strtmp) + 1;
	memset(dllEncryptFiles,0,2048*sizeof(char));
	memcpy(dllEncryptFiles, strtmp, len);
	(*env)->ReleaseStringUTFChars(env, encryptFiles, strtmp);

	g_message("Get dllEncryptFiles on android %s", dllEncryptFiles); 

	int in    = 0;
        char *buf = dllEncryptFiles;
        while((dllEncryptFilesArr[in] = strtok(buf, ",")) != NULL)
        {   
		g_message("dllEncryptFilesArr on android %d : %s", in,dllEncryptFilesArr[in]);
		in++;
		buf = NULL;
        }
	encrptCount = in;
	
	g_message("dllEncryptFilesArr count on android %d", encrptCount);
    }
...
mono_image_open_from_data_with_name (char *data, guint32 data_len, gboolean need_copy, MonoImageOpenStatus *status, gboolean refonly, const char *name)
{
	...
	g_message("mono_image_open_from_data_with_name enableEncrypt %d",enableEncrypt);
	...
	if(enableEncrypt)
	{
		if(name)
		{
			for(k = 0; k < encrptCount; k++)
			{
				if(strstr(name,dllEncryptFilesArr[k]))
				{
			...
			g_message("mono_image_open_from_data_with_name need decrypt %d name %s",eFlag,name);

			if(eFlag)
			{
				g_message("mono_image_open_from_data_with_name  decrypt start name %s",name);
				...
				g_message("mono_image_open_from_data_with_name  decrypt end name %s",name);
			}
		}
